...
func Append(ctx context.Context, dwh destination.DataWarehouse, tableData *optimization.TableData, opts types.AdditionalSettings) error {
	...
	diffColumnsResult := columns.DiffAndFilter(
	...
	if tableConfig.CreateTable() {
		if err = CreateTable(ctx, dwh, tableData, tableConfig, opts.ColumnSettings, tableID, false, diffColumnsResult.TargetColumnsMissing); err != nil {
			return fmt.Errorf("failed to create table: %w", err)
		}
	} else {
		if err = AlterTableAddColumns(ctx, dwh, tableConfig, opts.ColumnSettings, tableID, diffColumnsResult.TargetColumnsMissing); err != nil {
