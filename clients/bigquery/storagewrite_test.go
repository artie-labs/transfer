...
func newDecimal(value float64) *apd.Decimal {
	out, err := new(apd.Decimal).SetFloat64(value)
	if err != nil {
		panic(err)
	}
	return out
...
func TestRowToMessage(t *testing.T) {
	...
	row := map[string]any{
		...
		"c_numeric":        decimal.NewDecimal(nil, 5, newDecimal(3.1415926)),
		"c_string":         "foo bar",
		"c_string_decimal": decimal.NewDecimal(nil, 5, newDecimal(1.618033)),
