...
func (s *SnowflakeTestSuite) TestMutateColumnsWithMemoryCacheDeletions() {
	...
	val := tc.ShouldDeleteColumn(s.ctx, nameCol.Name(sql.DoNotEscapeNameArgs), time.Now().Add(-1*6*time.Hour), true)
...
func (s *SnowflakeTestSuite) TestShouldDeleteColumn() {
	...
	allowed := s.stageStore.configMap.TableConfig(fqName).ShouldDeleteColumn(s.ctx, nameCol.Name(sql.DoNotEscapeNameArgs),
	...
	allowed = s.stageStore.configMap.TableConfig(fqName).ShouldDeleteColumn(s.ctx, nameCol.Name(sql.DoNotEscapeNameArgs), time.Now(), true)
