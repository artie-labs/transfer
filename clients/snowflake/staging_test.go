...
func (s *SnowflakeTestSuite) TestPrepareTempTable() {
	...
	randomFileName, err := tempTableID.RandomFileName()
	assert.NoError(s.T(), err)
...

	{
		...
		putQueryRegex := regexp.QuoteMeta(fmt.Sprintf(`PUT 'file://%s' @"DATABASE"."SCHEMA".%s`, filepath.Join(os.TempDir(), randomFileName), stagingTableID.EscapedTable()))
		s.mockDB.ExpectExec(putQueryRegex).WillReturnResult(sqlmock.NewResult(0, 0))

		resourceName := addPrefixToTableName(tempTableID, "%")
		copyQueryRegex := regexp.QuoteMeta(fmt.Sprintf(`COPY INTO %s ("USER_ID","FIRST_NAME","LAST_NAME","DUSTY") FROM (SELECT $1,$2,$3,$4 FROM @%s) FILES = ('%s')`, tempTableName, resourceName, randomFileName))
...
func (s *SnowflakeTestSuite) TestLoadTemporaryTable() {
	...
	assert.NoError(s.T(), err)
	assert.True(s.T(), strings.HasPrefix(file.FileName, fmt.Sprintf("%s_", strings.ReplaceAll(tempTableID.FullyQualifiedName(), `"`, ""))))
	assert.True(s.T(), strings.HasSuffix(file.FileName, ".csv.gz"))
