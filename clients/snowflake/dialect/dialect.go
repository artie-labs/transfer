...
func (sd SnowflakeDialect) BuildIsNotToastValueExpression(tableAlias string, column columns.Column) string {
...
func (sd SnowflakeDialect) BuildMergeQueries(
	...
	equalitySQLParts := sql.BuildColumnComparisons(primaryKeys, TargetAlias, constants.StagingAlias, sql.Equal, sd)
...
WHEN NOT MATCHED AND IFNULL(%s.%s, false) = false THEN INSERT (%s) VALUES (%s);`,
			...
			idempotentClause, sql.BuildColumnsUpdateFragment(cols, constants.StagingAlias, TargetAlias, sd),
			// Insert
...
WHEN NOT MATCHED AND IFNULL(%s.%s, false) = false THEN INSERT (%s) VALUES (%s);`,
		...
		// Update
		constants.StagingAlias, sd.QuoteIdentifier(constants.DeleteColumnMarker), idempotentClause, sql.BuildColumnsUpdateFragment(cols, constants.StagingAlias, TargetAlias, sd),
		// Insert
		constants.StagingAlias, sd.QuoteIdentifier(constants.DeleteColumnMarker), strings.Join(sql.QuoteColumns(cols, sd), ","),
		array.StringsJoinAddPrefix(array.StringsJoinAddPrefixArgs{
			Vals:      sql.QuoteColumns(cols, sd),
