...
func (p batchProcessArgs) process(ctx context.Context, cfg config.Config, inMemDB *models.DatabaseData, dest destination.Destination, metricsClient base.Client) error {
	...
	for _, msg := range p.Msgs {
		...
		pkMap, err := topicConfig.buildPKMap(msg.Key(), reservedColumns)
		if err != nil {
			tags["what"] = "marshall_pk_err"
			return fmt.Errorf("cannot unmarshal key %q: %w", string(msg.Key()), err)
		}

		_event, err := topicConfig.GetEventFromBytes(msg.Value())
		if err != nil {
		...
		_, _, err = evt.Save(cfg, inMemDB, topicConfig.tc, reservedColumns)
		if err != nil {
			tags["what"] = "save_fail"
	...
	err := FlushSingleTopic(ctx, inMemDB, dest, metricsClient, p.WhClient, Args{Reason: "flushOnReceive", ReportDBExecutionTime: cfg.Reporting.EmitDBExecutionTime}, topicConfig.tc.Topic, false)
	if err != nil {
