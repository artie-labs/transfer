...
func (p processArgs) process(ctx context.Context, cfg config.Config, inMemDB *models.DatabaseData, dest destination.Destination, metricsClient base.Client) (cdc.TableID, error) {
	...
	if len(p.Msg.Key()) == 0 && len(topicConfig.tc.PrimaryKeysOverride) > 0 {
		// If the message key is empty (e.g. table without a primary key) but PrimaryKeysOverride is configured,
		// the primary key values will be extracted from the event payload instead.
