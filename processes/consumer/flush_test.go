...
func (f *FlushTestSuite) TestMemoryBasic() {
	...
	for i := range 5 {
		...
		evt, err := event.ToMemoryEvent(context.Background(), f.baseline, mockEvent, map[string]any{"id": fmt.Sprintf("pk-%d", i)}, topicConfig, config.Replication)
...
func (f *FlushTestSuite) TestShouldFlush() {
	...
	for i := range int(float64(f.cfg.BufferRows) * 1.5) {
		...
		evt, err := event.ToMemoryEvent(context.Background(), f.baseline, mockEvent, map[string]any{"id": fmt.Sprintf("pk-%d", i)}, kafkalib.TopicConfig{}, config.Replication)
		assert.NoError(f.T(), err)
	...
	for idx := range tableIDs {
		...
		go func(tableID cdc.TableID) {
			...
			for i := range 5 {
				...
				evt, err := event.ToMemoryEvent(context.Background(), f.baseline, mockEvent, map[string]any{"id": fmt.Sprintf("pk-%d", i)}, kafkalib.TopicConfig{Schema: tableID.Schema, Topic: topicConfig.Topic}, config.Replication)
				assert.NoError(f.T(), err)
