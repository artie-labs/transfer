...
func (e *EventsTestSuite) TestEvent_TableName() {
	{
		...
		evt, err := ToMemoryEvent(context.Background(), e.fakeBaseline, e.fakeEvent, idMap, kafkalib.TopicConfig{}, config.Replication)
		assert.NoError(e.T(), err)
		assert.Equal(e.T(), e.fakeEvent.GetTableName(), evt.GetTable())
	}
	{
		// Now pass it in, it should override.
		evt, err := ToMemoryEvent(context.Background(), e.fakeBaseline, e.fakeEvent, idMap, kafkalib.TopicConfig{TableName: "orders"}, config.Replication)
		assert.NoError(e.T(), err)
		assert.Equal(e.T(), "orders", evt.GetTable())
	}
	{
		// Now, if it's history mode...
		evt, err := ToMemoryEvent(context.Background(), e.fakeBaseline, e.fakeEvent, idMap, kafkalib.TopicConfig{TableName: "orders"}, config.History)
		assert.NoError(e.T(), err)
		assert.Equal(e.T(), "orders__history", evt.GetTable())

		// Table already has history suffix, so it won't add extra.
		evt, err = ToMemoryEvent(context.Background(), e.fakeBaseline, e.fakeEvent, idMap, kafkalib.TopicConfig{TableName: "dusty__history"}, config.History)
		assert.NoError(e.T(), err)
...
func (e *EventsTestSuite) TestEvent_Columns() {
	{
		evt, err := ToMemoryEvent(context.Background(), e.fakeBaseline, e.fakeEvent, map[string]any{"id": 123}, kafkalib.TopicConfig{}, config.Replication)
		assert.NoError(e.T(), err)
	...
	}
	{
		...
		evt, err := ToMemoryEvent(context.Background(), e.fakeBaseline, e.fakeEvent, map[string]any{"id": 123, "CAPITAL": "foo"}, kafkalib.TopicConfig{}, config.Replication)
		assert.NoError(e.T(), err)
	...
	}
	{
		...
		evt, err := ToMemoryEvent(context.Background(), e.fakeBaseline, e.fakeEvent, map[string]any{"id": 123}, kafkalib.TopicConfig{}, config.History)
		assert.NoError(e.T(), err)
...
func (e *EventsTestSuite) TestEventPrimaryKeys() {
	...
	anotherEvt, err := ToMemoryEvent(context.Background(), e.fakeBaseline, mockEvent, map[string]any{"id": 1, "course_id": 2}, kafkalib.TopicConfig{}, config.Replication)
	assert.NoError(e.T(), err)
	...
	// If the value doesn't exist in the event payload
	{
		...
		evt, err := ToMemoryEvent(context.Background(), e.fakeBaseline, mockEvent, map[string]any{"id": 123}, kafkalib.TopicConfig{}, config.Replication)
		assert.NoError(e.T(), err)
...
func (e *EventsTestSuite) TestPrimaryKeyValueDeterministic() {
	...
	evt, err := ToMemoryEvent(context.Background(), e.fakeBaseline, mockEvent, map[string]any{
		"aa":    1,
		"bb":    5,
		"zz":    "ff",
		"gg":    "artie",
...
func (e *EventsTestSuite) TestEvent_PrimaryKeysOverride() {
	{
		...
		evt, err := ToMemoryEvent(context.Background(), e.fakeBaseline, e.fakeEvent, map[string]any{"not_id": 123}, kafkalib.TopicConfig{}, config.Replication)
		assert.NoError(e.T(), err)
		assert.Equal(e.T(), []string{"not_id"}, evt.GetPrimaryKeys())
	}
	{
		// Specified primary keys override
		evt, err := ToMemoryEvent(context.Background(), e.fakeBaseline, e.fakeEvent, map[string]any{"not_id": 123}, kafkalib.TopicConfig{PrimaryKeysOverride: []string{"id"}}, config.Replication)
		assert.NoError(e.T(), err)
...
func (e *EventsTestSuite) TestEvent_StaticColumns() {
	{
		...
		_, err := ToMemoryEvent(context.Background(), e.fakeBaseline, e.fakeEvent, map[string]any{"id": 123}, kafkalib.TopicConfig{StaticColumns: []kafkalib.StaticColumn{{Name: "id", Value: "123"}}}, config.Replication)
		assert.ErrorContains(e.T(), err, `static column "id" collides with event data`)
	}
	{
		// No error since there's no collision
		e.fakeEvent.GetDataReturns(map[string]any{"id": 123}, nil)
		evt, err := ToMemoryEvent(context.Background(), e.fakeBaseline, e.fakeEvent, map[string]any{"id": 123}, kafkalib.TopicConfig{StaticColumns: []kafkalib.StaticColumn{{Name: "foo", Value: "bar"}}}, config.Replication)
		assert.NoError(e.T(), err)
...
func (e *EventsTestSuite) TestToMemoryEventWithSoftPartitioning() {
	...
	for _, freq := range partitionFrequencies {
		...
		event, err := ToMemoryEvent(context.Background(), e.fakeBaseline, mockEvent, map[string]any{"id": "123"}, tc, config.Replication)
		assert.NoError(e.T(), err)
