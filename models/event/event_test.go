...
func (e *EventsTestSuite) TestEvent_TableName() {
	{
		...
		evt, err := ToMemoryEvent(e.T().Context(), e.fakeBaseline, e.fakeEvent, idMap, kafkalib.TopicConfig{}, config.Replication, config.SharedDestinationSettings{}, nil, nil)
		assert.NoError(e.T(), err)
		assert.Equal(e.T(), e.fakeEvent.GetTableName(), evt.GetTable())
	}
	{
		// Now pass it in, it should override.
		evt, err := ToMemoryEvent(e.T().Context(), e.fakeBaseline, e.fakeEvent, idMap, kafkalib.TopicConfig{TableName: "orders"}, config.Replication, config.SharedDestinationSettings{}, nil, nil)
		assert.NoError(e.T(), err)
		assert.Equal(e.T(), "orders", evt.GetTable())
	}
	{
		// Now, if it's history mode...
		evt, err := ToMemoryEvent(e.T().Context(), e.fakeBaseline, e.fakeEvent, idMap, kafkalib.TopicConfig{TableName: "orders"}, config.History, config.SharedDestinationSettings{}, nil, nil)
		assert.NoError(e.T(), err)
		assert.Equal(e.T(), "orders__history", evt.GetTable())

		// Table already has history suffix, so it won't add extra.
		evt, err = ToMemoryEvent(e.T().Context(), e.fakeBaseline, e.fakeEvent, idMap, kafkalib.TopicConfig{TableName: "dusty__history"}, config.History, config.SharedDestinationSettings{}, nil, nil)
		assert.NoError(e.T(), err)
...
func (e *EventsTestSuite) TestEvent_Columns() {
	{
		...
		evt, err := ToMemoryEvent(e.T().Context(), e.fakeBaseline, e.fakeEvent, map[string]any{"id": 123}, kafkalib.TopicConfig{}, config.Replication, config.SharedDestinationSettings{}, nil, nil)
		assert.NoError(e.T(), err)
	...
	{
		e.fakeEvent.GetColumnsReturns([]columns.Column{}, nil)
		evt, err := ToMemoryEvent(e.T().Context(), e.fakeBaseline, e.fakeEvent, map[string]any{"id": 123, "capital": "foo"}, kafkalib.TopicConfig{}, config.Replication, config.SharedDestinationSettings{}, nil, nil)
		assert.NoError(e.T(), err)
	...
	}
	{
		...
		evt, err := ToMemoryEvent(e.T().Context(), e.fakeBaseline, e.fakeEvent, map[string]any{"id": 123}, kafkalib.TopicConfig{}, config.History, config.SharedDestinationSettings{}, nil, nil)
		assert.NoError(e.T(), err)
...
func (e *EventsTestSuite) TestEventPrimaryKeys() {
	...
	anotherEvt, err := ToMemoryEvent(e.T().Context(), e.fakeBaseline, mockEvent, map[string]any{"id": 1, "course_id": 2}, kafkalib.TopicConfig{}, config.Replication, config.SharedDestinationSettings{}, nil, nil)
	assert.NoError(e.T(), err)
	...
	// If the value doesn't exist in the event payload
	{
		...
		evt, err := ToMemoryEvent(e.T().Context(), e.fakeBaseline, mockEvent, map[string]any{"id": 123}, kafkalib.TopicConfig{}, config.Replication, config.SharedDestinationSettings{}, nil, nil)
		assert.NoError(e.T(), err)
...
func (e *EventsTestSuite) TestPrimaryKeyValueDeterministic() {
	...
	evt, err := ToMemoryEvent(e.T().Context(), e.fakeBaseline, mockEvent, map[string]any{
		...
		"gg":    "artie",
		"dusty": "mini aussie",
	}, kafkalib.TopicConfig{}, config.Replication, config.SharedDestinationSettings{}, nil, nil)
	assert.NoError(e.T(), err)
...
func (e *EventsTestSuite) TestEvent_PrimaryKeysOverride() {
	{
		...
		evt, err := ToMemoryEvent(e.T().Context(), e.fakeBaseline, e.fakeEvent, map[string]any{"not_id": 123}, kafkalib.TopicConfig{}, config.Replication, config.SharedDestinationSettings{}, nil, nil)
		assert.NoError(e.T(), err)
		assert.Equal(e.T(), []string{"not_id"}, evt.GetPrimaryKeys())
	}
	{
		// Specified primary keys override
		evt, err := ToMemoryEvent(e.T().Context(), e.fakeBaseline, e.fakeEvent, map[string]any{"not_id": 123}, kafkalib.TopicConfig{PrimaryKeysOverride: []string{"id"}}, config.Replication, config.SharedDestinationSettings{}, nil, nil)
		assert.NoError(e.T(), err)
...
func (e *EventsTestSuite) TestEvent_StaticColumns() {
	{
		...
		_, err := ToMemoryEvent(e.T().Context(), e.fakeBaseline, e.fakeEvent, map[string]any{"id": 123}, kafkalib.TopicConfig{StaticColumns: []kafkalib.StaticColumn{{Name: "id", Value: "123"}}}, config.Replication, config.SharedDestinationSettings{}, nil, nil)
		assert.ErrorContains(e.T(), err, `static column "id" collides with event data`)
	}
	{
		// No error since there's no collision
		e.fakeEvent.GetDataReturns(map[string]any{"id": 123}, nil)
		evt, err := ToMemoryEvent(e.T().Context(), e.fakeBaseline, e.fakeEvent, map[string]any{"id": 123}, kafkalib.TopicConfig{StaticColumns: []kafkalib.StaticColumn{{Name: "foo", Value: "bar"}}}, config.Replication, config.SharedDestinationSettings{}, nil, nil)
		assert.NoError(e.T(), err)
...
func (e *EventsTestSuite) TestToMemoryEventWithSoftPartitioning() {
	...
	for _, freq := range partitionFrequencies {
		...
		event, err := ToMemoryEvent(e.T().Context(), e.fakeBaseline, mockEvent, map[string]any{"id": "123"}, tc, config.Replication, config.SharedDestinationSettings{}, nil, lib.NewKVCache[string]())
		assert.NoError(e.T(), err)
