...
func (e *EventsTestSuite) TestSaveEvent() {
	...
	event, err := ToMemoryEvent(e.T().Context(), e.fakeBaseline, mockEvent, map[string]any{"id": "123"}, topicConfig, config.Replication, config.SharedDestinationSettings{}, nil, nil)
...
func (e *EventsTestSuite) TestEvent_SaveColumnsNoData() {
	...
	evt, err := ToMemoryEvent(e.T().Context(), e.fakeBaseline, mockEvent, map[string]any{"col_1": "123"}, topicConfig, config.Replication, config.SharedDestinationSettings{}, nil, nil)
	assert.NoError(e.T(), err)
...
func (e *EventsTestSuite) TestEventSaveAppendOnlyMode() {
	{
		...
		event, err := ToMemoryEvent(e.T().Context(), e.fakeBaseline, mockEvent, map[string]any{"id": "123"}, appendOnlyTC, config.Replication, config.SharedDestinationSettings{}, nil, nil)
		assert.NoError(e.T(), err)
	...
	}
	{
		...
		event, err := ToMemoryEvent(e.T().Context(), e.fakeBaseline, mockEvent, map[string]any{"id": "456"}, appendOnlyWithSoftDeleteTC, config.Replication, config.SharedDestinationSettings{}, nil, nil)
		assert.NoError(e.T(), err)
