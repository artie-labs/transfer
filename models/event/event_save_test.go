...
func (e *EventsTestSuite) TestSaveEvent() {
	...
	event, err := ToMemoryEvent(e.T().Context(), e.fakeBaseline, mockEvent, []string{"id"}, topicConfig, config.Replication)
...
func (e *EventsTestSuite) TestEvent_SaveColumnsNoData() {
	...
	evt, err := ToMemoryEvent(e.T().Context(), e.fakeBaseline, mockEvent, []string{"col_1"}, topicConfig, config.Replication)
	assert.NoError(e.T(), err)
