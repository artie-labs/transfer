...
func shouldSkipColumn(colName string, softDelete bool, includeArtieUpdatedAt bool, includeDatabaseUpdatedAt bool, includeArtieOperation bool, mode config.Mode) bool {
	...
	if colName == constants.OperationColumnMarker {
		if mode == config.History || includeArtieOperation {
			return false
		}
...
func filterColumns(columns []Column, softDelete bool, includeArtieUpdatedAt bool, includeDatabaseUpdatedAt bool, includeArtieOperation bool, mode config.Mode) []Column {
	var filteredColumns []Column
	for _, col := range columns {
		if shouldSkipColumn(col.Name(), softDelete, includeArtieUpdatedAt, includeDatabaseUpdatedAt, includeArtieOperation, mode) {
			continue
		}
...
func DiffAndFilter(columnsInSource []Column, columnsInDestination []Column, softDelete bool, includeArtieUpdatedAt bool, includeDatabaseUpdatedAt bool, includeArtieOperation bool, mode config.Mode) ([]Column, []Column) {
	diffResult := Diff(columnsInSource, columnsInDestination)
	return filterColumns(diffResult.SourceColumnsMissing, softDelete, includeArtieUpdatedAt, includeDatabaseUpdatedAt, includeArtieOperation, mode),
		filterColumns(diffResult.TargetColumnsMissing, softDelete, includeArtieUpdatedAt, includeDatabaseUpdatedAt, includeArtieOperation, mode)
}
