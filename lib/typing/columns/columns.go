...
func (c *Column) Name(destKind constants.DestinationKind) string {
	return sql.EscapeNameIfNecessary(c.name, destKind)
}
...
func (c *Columns) GetEscapedColumnsToUpdate(destKind constants.DestinationKind) []string {
	if c == nil {
		return []string{}
	}
	...
	for _, col := range c.columns {
		...
		cols = append(cols, col.Name(destKind))
	}
...
func (c *Columns) UpdateQuery(destKind constants.DestinationKind, skipDeleteCol bool) string {
	var cols []string
