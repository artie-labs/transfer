...
func ParseNumeric(parts []string) (KindDetails, error) {
	...
	for _, part := range parts {
		...
		if err != nil {
			return Invalid, fmt.Errorf("failed to parse number, part %q: %w", part, err)
		}
	...
	if len(parsedNumbers) == 1 || parsedNumbers[1] == 0 {
		return NewDecimalDetailsFromTemplate(EDecimal, decimal.NewDetails(parsedNumbers[0], 0)), nil
	}

	return NewDecimalDetailsFromTemplate(EDecimal, decimal.NewDetails(parsedNumbers[0], parsedNumbers[1])), nil
}
