...
func TestParseVersion(t *testing.T) {
	{
		...
		assert.Equal(t, 16, version.Major)
	...
	}
	{
		...
		assert.NoError(t, err)
		assert.Equal(t, 13, version.Major)
		assert.Equal(t, 15, version.Minor)
	}
	{
		...
		assert.NoError(t, err)
		assert.Equal(t, 14, version.Major)
	...
	}
	{
		...
		assert.NoError(t, err)
		assert.Equal(t, 15, version.Major)
