...
func TestRestCatalog_Validate(t *testing.T) {
	{
		// missing uri
		rc := RestCatalog{}
		assert.ErrorContains(t, rc.Validate(), "rest catalog uri is required")
	}
	{
		// missing warehouse
		rc := RestCatalog{
		...
		assert.ErrorContains(t, rc.Validate(), "rest catalog warehouse is required")
	}
	{
		// missing token and credential
		rc := RestCatalog{
			URI:       "http://localhost:8181",
			Warehouse: "my_catalog",
		}
		assert.ErrorContains(t, rc.Validate(), "rest catalog requires either token or credential for authentication")
	}
	{
		// missing bucket
		rc := RestCatalog{
		...
		assert.ErrorContains(t, rc.Validate(), "rest catalog bucket is required for staging files")
	}
	{
		// missing AWS access key ID
		rc := RestCatalog{
		...
		assert.ErrorContains(t, rc.Validate(), "rest catalog awsAccessKeyID is required")
	}
	{
		// missing AWS secret access key
		rc := RestCatalog{
		...
		assert.ErrorContains(t, rc.Validate(), "rest catalog awsSecretAccessKey is required")
	}
	{
		// missing region
		rc := RestCatalog{
		...
		assert.ErrorContains(t, rc.Validate(), "rest catalog region is required")
	}
	{
		// valid with token
		rc := RestCatalog{
