...
func (t TableIdentifier) FqName(kind constants.DestinationKind, escape bool, uppercaseEscNames bool, opts FqNameOpts) string {
	...
	case constants.S3:
		...
		return fmt.Sprintf("%s.%s.%s", t.database, t.schema, t.escapeName(uppercaseEscNames, &sql.NameArgs{
			Escape:   false,
			DestKind: kind,
		}))
	case constants.Redshift:
		...
		return fmt.Sprintf("%s.%s", t.schema, t.escapeName(uppercaseEscNames, &sql.NameArgs{
			Escape:   escape,
			DestKind: kind,
		}))
	case constants.MSSQL:
		return fmt.Sprintf("%s.%s", stringutil.Override(t.schema, opts.MsSQLSchemaOverride), t.escapeName(uppercaseEscNames, &sql.NameArgs{
			Escape:   escape,
			DestKind: kind,
		}))
	case constants.BigQuery:
		...
		return fmt.Sprintf("`%s`.`%s`.%s", opts.BigQueryProjectID, t.database, t.escapeName(uppercaseEscNames, &sql.NameArgs{
			Escape:   escape,
			DestKind: kind,
		}))
	default:
