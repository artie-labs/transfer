...
func (d *DDLTestSuite) TestAlterTableDropColumnsBigQuery() {
	...
	assert.Equal(d.T(), originalColumnLength, len(d.bigQueryStore.GetConfigMap().TableConfig(fqName).ReadOnlyColumns().GetColumns()), d.bigQueryStore.GetConfigMap().TableConfig(fqName).ReadOnlyColumns())
	...
	assert.Equal(d.T(), 0, len(d.bigQueryStore.GetConfigMap().TableConfig(fqName).ReadOnlyColumnsToDelete()), d.bigQueryStore.GetConfigMap().TableConfig(fqName).ReadOnlyColumnsToDelete())
	// Columns have not been deleted yet.
	assert.Equal(d.T(), 0, len(d.bigQueryStore.GetConfigMap().TableConfig(fqName).ReadOnlyColumns().GetColumns()), d.bigQueryStore.GetConfigMap().TableConfig(fqName).ReadOnlyColumns())
...
func (d *DDLTestSuite) TestAlterTableAddColumns() {
	...
	assert.Equal(d.T(), 0, len(d.bigQueryStore.GetConfigMap().TableConfig(fqName).ReadOnlyColumnsToDelete()), d.bigQueryStore.GetConfigMap().TableConfig(fqName).ReadOnlyColumnsToDelete())
	assert.Equal(d.T(), len(existingCols.GetColumns()), len(d.bigQueryStore.GetConfigMap().TableConfig(fqName).ReadOnlyColumns().GetColumns()), d.bigQueryStore.GetConfigMap().TableConfig(fqName).ReadOnlyColumns())
	...
	assert.Equal(d.T(), newColsLen+existingColsLen, len(d.bigQueryStore.GetConfigMap().TableConfig(fqName).ReadOnlyColumns().GetColumns()), d.bigQueryStore.GetConfigMap().TableConfig(fqName).ReadOnlyColumns())
	// Check by iterating over the columns
	for _, column := range d.bigQueryStore.GetConfigMap().TableConfig(fqName).ReadOnlyColumns().GetColumns() {
...
func (d *DDLTestSuite) TestAlterTableAddColumnsSomeAlreadyExist() {
	...
	// Check all the columns, make sure it's correct. (length)
	assert.Equal(d.T(), existingColsLen, len(d.bigQueryStore.GetConfigMap().TableConfig(fqName).ReadOnlyColumns().GetColumns()), d.bigQueryStore.GetConfigMap().TableConfig(fqName).ReadOnlyColumns())
	// Check by iterating over the columns
	for _, column := range d.bigQueryStore.GetConfigMap().TableConfig(fqName).ReadOnlyColumns().GetColumns() {
		existingCol, isOk := existingCols.GetColumn(column.Name(nil))
		assert.True(d.T(), isOk)
...
func (d *DDLTestSuite) TestAlterTableDropColumnsBigQuerySafety() {
	...
	assert.Equal(d.T(), originalColumnLength, len(d.bigQueryStore.GetConfigMap().TableConfig(fqName).ReadOnlyColumns().GetColumns()))
